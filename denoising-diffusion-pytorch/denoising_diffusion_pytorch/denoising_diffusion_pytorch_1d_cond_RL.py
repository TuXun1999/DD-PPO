...
class Trainer1DCondRL(object):
    def __init__(
        ...
        *,
        ...
        self.Img_as_global_cond = Img_as_global_cond
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    # global cond encoder
                    if self.Img_as_global_cond:
                        # reshape B, T, ... to B*T
                        batch_size = global_cond.shape[0]
                        global_cond = global_cond.reshape(-1,*global_cond.shape[2:])

                        # obs_encoder needs a dict()
                        obs_dict = dict()
                        obs_dict["overhead_img"] = global_cond
                        nobs_features = self.obs_encoder(obs_dict)

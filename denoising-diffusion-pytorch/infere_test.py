...
from denoising_diffusion_pytorch import ConditionalUnet1D, \
    GaussianDiffusion1DConditional, Trainer1DCond, Dataset1DCond, GaussianDiffusion1DConditionalRL, \
    Trainer1DCondRL
...
class MoveCubeEnv():
    ...
    def collect_poses(self):
        ...
        while True and not self.kill_thread.is_set():
            ...
            rgb_1, depth_1, _, _ = self.cam1.render(rgb=True, depth=False, segmentation=False, normal=False)
    ...
    def diffusion_policy(self, **kwargs):
        ...
        print('device in obs encoder: ', device)
        ...
        if option == "unet1d":
            model = ConditionalUnet1D(
                ...
                local_cond_dim = 1,
                global_cond_dim = obs_feature_dim * obs_dim,
            )
        else:
            # model = ConditionalUnet1D(
            #     input_dim = 6,
        ...
        results_folder = '../collected_demos/T-shape/grasp_pose5/results'
        ...
        for n in range(obs_length):
            rgb_2, depth_2, _, _ = self.cam2.render(rgb=True, depth=False, segmentation=False, normal=False)
            img_data.append(rgb_2)
        
        img_data = np.array(img_data)
        img_data = np.transpose(img_data, (0, 3, 1, 2))
        img_cond = torch.from_numpy(np.float32(img_data))
        img_cond = img_cond.unsqueeze(0)
        # img_cond = torch.zeros_like(img_cond) 
        
        print('img cond shape: ', img_cond.shape)
        ...
        while True:
            ...
            img_steps = img_cond.shape[0]
            img_cond = img_cond.reshape(-1,*img_cond.shape[2:])

            # obs_encoder needs a dict()
            obs_dict = dict()
            obs_dict["overhead_img"] = img_cond
            nobs_features = obs_encoder(obs_dict)
            # reshape back to B, Do
            img_cond = nobs_features.reshape(img_steps, -1)
            
            print(f"img_cond shape after reshape: {img_cond.shape}")
            print(f"local_label_sample shape: {local_label_sample.shape}")
            
            # Sample the sequence
            sampled_seq = diffusion.sample(batch_size = batch_size_sample, \
                    local_cond = local_label_sample, global_cond = img_cond)
...
args = {
    'policy': 'diffusion',
    'normalization_stats': '../collected_demos/T-shape/grasp_pose5/training_stats.pth',
}
env.policy(**args)

...
if [[ $PWD = *DD-PPO ]]; then
    cd docker
elif [[ ! $PWD = *DD-PPO/docker ]]; then
    echo -e "You must be in either 'DD-PPO' or the sub-directory 'docker' to run this command."
    return 1
fi
...
if ! $gui && nvidia; then
    docker run -it\
           --volume $(pwd):/home/$USER/repo/DD-PPO/\
